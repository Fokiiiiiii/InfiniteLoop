...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
    ...
    internal class TaskModule
    {
        ...
        public static void FinishTaskRequestHandler(Session session, Packet.Request packet)
        {
            ...
            PrepareTaskRequest(session, bianca, theatre, theatre5, theatre4);
            if (theatre5)
            {
        ...
        internal static void RecordTableDrivenProgress(Session session, IEnumerable<(int ConditionType, int? Parameter, int Amount)> increments, bool sendNotification = true, TheatreModule.Mutation? theatre = null, Theatre5Module.Mutation? theatre5 = null, Theatre4Module.Mutation? theatre4 = null)
        {
            ...
            Dictionary<int, int> currentAmounts = CurrentConditionsById.Value.Values
                ...
                .Where(condition => condition.Amount > 0)
                .ToDictionary(condition => condition.Id, condition => condition.Amount);
            RecordConditionAmounts(session, conditionAmounts, currentAmounts, sendNotification, theatre, theatre5, theatre4);
        }

        private static void RecordConditionAmounts(Session session, Dictionary<int, int> conditionAmounts, Dictionary<int, int> currentAmounts, bool sendNotification = true, TheatreModule.Mutation? theatre = null, Theatre5Module.Mutation? theatre5 = null, Theatre4Module.Mutation? theatre4 = null)
        {
            if (conditionAmounts.Count == 0 && currentAmounts.Count == 0)
                return;
            if (theatre is null && theatre5 is null && theatre4 is null) EnsureMissionResets(session);
            DateTimeOffset now = DateTimeOffset.UtcNow;
            List<TaskTable> tasks = TableReaderV2.Parse<TaskTable>()
            ...
            if (theatre is not null || theatre5 is not null || theatre4 is not null)
            {
                Action<int, int> add = theatre is not null ? theatre.AddTaskConditionProgress
                    : theatre5 is not null ? theatre5.AddTaskConditionProgress : theatre4!.AddTaskConditionProgress;
                Func<int, int> get = theatre is not null ? theatre.GetTaskConditionProgress
                    : theatre5 is not null ? theatre5.GetTaskConditionProgress : theatre4!.GetTaskConditionProgress;
                Action<NotifyTask> push = theatre is not null ? theatre.Push
                    : theatre5 is not null ? theatre5.Push : theatre4!.Push;
        ...
        private static void PrepareTaskRequest(Session session, bool bianca, bool theatre, bool theatre5 = false, bool theatre4 = false)
        {
